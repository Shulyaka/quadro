...
bool math_magic(fixed p[10][3], fixed x[9])          //finds the best calibration parameters using the least squires mathod and iterational Newton's method
{
  byte n=10;
  fixed x2[9];
  fixed r[9];
  ...
  while(norm>=eps)
  {
    ...
    for(i=0; i<9; i++)
      for(j=0; j<9; j++)
      {
        a[i][j]=0;
        for(k=0; k<n; k++)
          a[i][j]=a[i][j] + p2[k][i/3] * pow(p[k][i/3], i%3) * p2[k][j/3] * pow(p[k][j/3], j%3);    //J'*J/4
      }
...
//        for(j=0; j<3; j++)
//          for(l=0; l<9; l+=3)
//            a[i+l][j+l]=a[i+l][j+l]+( (f[k] * pow(p[k][l/3], i+j)) >> 1);    //f(i)*H(i)/4, i=0..n

    disable_sensor_interrupts(); //for speed
    if(!lsolve_float(a, r, x2))
    {
...
bool math_magic(float p[10][3], float x[9])          //finds the best calibration parameters using the least squires mathod and iterational Newton's method
{
  byte n=10;
  float x2[9];
  float r[9];
  ...
  while(norm>=eps)
  {
    ...
    for(j=0; j<9; j++)
    {
      r[j]=0;
      for(i=0; i<n; i++)
        r[j]=r[j] - p2[i][j/3] * pow(p[i][j/3], j%3) * f[i];     //-J'*f/16
      r[j]=r[j]/8.0;
    }
    ...
    for(i=0; i<9; i++)
      for(j=0; j<9; j++)
      {
        a[i][j]=0;
        for(k=0; k<n; k++)
          a[i][j]=a[i][j] + p2[k][i/3] * pow(p[k][i/3], i%3) * p2[k][j/3] * pow(p[k][j/3], j%3);    //J'*J/16
        a[i][j]=a[i][j]/4.0;
      }
...
bool math_magic_float(fixed p[10][3], fixed x[9])
{
  const byte n=9;
  float p2[10][3];
  float x2[n];
...
*/
  ...
  if(!math_magic_float(point, k)) // find gain to be applied for the n points to be on a sphere
